...
export default function FirewallManager() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      alert(`${t('error_saving_rules')}: ${e instanceof Error ? e.message : t('unknown_error')}`)
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div className="bg-white rounded-2xl p-6 shadow-xl space-y-6">
      <div>
        <h2 className="text-3xl font-bold text-gray-800">{t('firewall_management')}</h2>
        <p className="text-gray-600 mt-2">{t('manage_access_rules')}</p>
      </div>
      ...
      {rules.length === 0 && !loading && (
        <div className="text-center py-12">
          ...
          <h3 className="mt-4 text-lg font-medium text-gray-900">{t('no_rules_configured')}</h3>
