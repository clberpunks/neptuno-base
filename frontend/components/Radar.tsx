...
export default function Radar() {
  ...
  const lineData = { labels: hoursRadar, datasets: [{ label:'Hits/Hora', data: hoursRadar.map(h => hitsByHourRadar[h]), fill:false }] };
        ...
        <div className="bg-white p-4 rounded-lg shadow grid grid-cols-1 md:grid-cols-2 gap-4">
          {limitPatterns.map(p => (
            <div key={p} className="text-center">
              ...
              <Doughnut data={{ labels:['Usado','Restante'], datasets:[{ data:[usageMap[p].used, usageMap[p].max-usageMap[p].used] }] }}/>
              <p className="mt-2 text-sm">{usageMap[p].used}/{usageMap[p].max} usadas</p>
            </div>
          ))}
        </div>
      )}
      ...
      <div className="bg-white p-4 rounded-lg shadow overflow-x-auto">
        ...
        <table className="min-w-full text-sm">
          <thead className="bg-gray-50">
            <tr>
              {['Fecha','IP','Ruta','Agente','Referer','UTM','JS','Fingerprint','Accept-Lang','Sec-CH-UA','Sec-CH-Mobile','Sec-CH-Platform','Estado','Regla','Redir URL'].map(h=>(
          ...
          <tbody>
            {logs.map(l => (
              <tr key={l.id} className="border-t">
                <td className="px-2 py-1 whitespace-nowrap">{new Date(l.timestamp).toLocaleString()}</td>
