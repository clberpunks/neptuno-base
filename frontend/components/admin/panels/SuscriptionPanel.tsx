...
useEffect(() => {
  ...
  const toggleStatus = async (id: string) => {
    await apiFetch(`/rest/admin/subscription-plans/${id}/toggle`, { method: 'PATCH' });
    fetchPlans();
  };

  const handleDelete = async (id: string) => {
    if (!confirm("¿Seguro que deseas eliminar este plan?")) return;
    await apiFetch(`/rest/admin/subscription-plans/${id}`, { method: 'DELETE' });
    fetchPlans();
  };
  ...
  return (
    <div className="bg-white rounded-xl p-6 shadow-sm">
      <h2 className="text-xl font-bold mb-4">Gestión de Planes de Suscripción</h2>
          ...
          {plans.map(p => (
            <tr key={p.id} className={`${!p.active ? 'opacity-40' : ''}`}>
              ...
              <td>
                {editingId === p.id
                  ? <input type="number" value={edited.traffic_limit ?? p.traffic_limit}
                           onChange={e => setEdited({ ...edited, traffic_limit: +e.target.value })}
                           className="border px-2 py-1 w-24 rounded" />
                  : p.traffic_limit.toLocaleString()}
              </td>
              <td>
                {editingId === p.id
                  ? <input type="number" value={edited.domain_limit ?? p.domain_limit}
                           onChange={e => setEdited({ ...edited, domain_limit: +e.target.value })}
                           className="border px-2 py-1 w-16 rounded" />
                  : p.domain_limit}
              </td>
              <td>
                {editingId === p.id
                  ? <input type="number" value={edited.user_limit ?? p.user_limit}
                           onChange={e => setEdited({ ...edited, user_limit: +e.target.value })}
                           className="border px-2 py-1 w-16 rounded" />
                  : p.user_limit}
              </td>
              <td>
                {editingId === p.id
                  ? <input type="number" value={edited.price ?? p.price}
                           onChange={e => setEdited({ ...edited, price: +e.target.value })}
                           className="border px-2 py-1 w-16 rounded" />
                  : `$${p.price}`}
              </td>
              <td>
                <span className={`font-medium ${p.active ? 'text-green-600' : 'text-gray-500'}`}>
              ...
              <td className="space-x-2">
                {editingId === p.id ? (
                  <>
                    <button onClick={handleSave} className="text-blue-600 hover:underline">Guardar</button>
                    <button onClick={() => setEditingId(null)} className="text-gray-500 hover:underline">Cancelar</button>
                  </>
                ) : (
                  <>
                    <button onClick={() => handleEdit(p)} className="text-indigo-600 hover:underline">Editar</button>
                    <button onClick={() => toggleStatus(p.id)} className="text-sm">
